...
class App_controller extends Controller{
  ...
  public function getDefi($f3){
      $donnees1=$this->model->getCats(array(
        'id_Cat'=>$f3->get('PARAMS.id_Cat'),
        'nom_Cat'=>$f3->get('PARAMS.nom_Cat')
        ));
      $f3->set('lescats', $donnees1);

      $donnees=$this->model->getDefigagne(array(
        'Pseudo'=>$f3->get('PARAMS.Pseudo'),
        'nomDefi'=>$f3->get('PARAMS.nomDefi'),
        'Question'=>$f3->get('PARAMS.Question'),
        'Image'=>$f3->get('PARAMS.Image'),
      ...
      /*appel du modèle User_model*/
      $modelName='User_model';
      $this->model=new $modelName();
      /*Permet de récupérer toutes les types de catégories de question
      route : /getDefi
      template : defi.html
      */
      $donnees2=$this->model->getUsers(array(
        'id_User'=>$f3->get('PARAMS.id_User'),
        'Pseudo'=>$f3->get('PARAMS.Pseudo')
        ));
  ...
  public function getReponse($f3){   
    $donnees=$this->model->getReponse(array(
      'id_Question'=>$f3->get('PARAMS.id_Question'),
      'Image'=>$f3->get('PARAMS.Image'),
      'Pseudo'=>$f3->get('PARAMS.Pseudo'),
      'Question'=>$f3->get('PARAMS.Question'),
  ...
  public function getMareponse($f3){ 
    $donnees=$this->model->getMa_reponse(array(
      'monid_Question'=>$f3->get('POST.id_question'),
      'id_Question'=>$f3->get('PARAMS.id_Question'),
      'Image'=>$f3->get('PARAMS.Image'),
      'Pseudo'=>$f3->get('PARAMS.Pseudo'),
      'Question'=>$f3->get('PARAMS.Question'),
  ...
  public function getForm_defi($f3){
   ...
   $this->model->getForm_defi(array(
    'id_User2'=>$f3->get('POST.a_qui'),
    'nomDefi'=>$f3->get('POST.nom_defi'),
    'Question'=>$f3->get('POST.question'),
    'Reponse'=>$f3->get('POST.reponse_voulue'),
    'id_User'=>$f3->get('SESSION.id'),
    'Pseudo'=>$f3->get('SESSION.Pseudo'),
    'id_Cat'=>$f3->get('POST.categories'),
    'Message'=>$f3->get('POST.msg_perso'),
    'Image'=>$f3->get('monfichier')
    ),$f3);
   $f3->reroute('/defi');
  }
  ...
  public function getUsersClassement($f3){
    $donnees=$this->model->getUsersClassement(array(
      'Pseudo'=>$f3->get('PARAMS.Pseudo'),
  ...
  public function getUser($f3){
    $f3->set('user',$this->model->getUser(array(
      'Id_User'=>$f3->get('PARAMS.Id_User')
      ))
    );
